...
class Bathtub(EndUse):
    ...
    def fct_frequency(self, age=None):
        ...
        distribution = getattr(np.random.RandomState(seed = self.random_state), f_stats['distribution'].lower()) # Change: Random Seeds 
    ...
    def simulate(self, consumption, users=None, ind_enduse=None, pattern_num=1, day_begin = '', day_end = '', random_state: int = None):  # Change: Random Seeds, Fix Duration
        ...
        for j, user in enumerate(users):
            ...
            for i in range(freq):
                ...
                prob_joint = normalize(prob_user[day_begin:day_end] * prob_usage)  # Change: Fix Duration 

                u = np.random.RandomState(seed = self.random_state).uniform()  # Change: Random Seeds 
                start = np.argmin(np.abs(np.cumsum(prob_joint) - u)) + day_begin
                end = start + duration
...
class BathroomTap(EndUse):
    ...
    def fct_duration_intensity(self):
        ...
        subtype = chooser(self.statistics['subtype'], 'penetration', random_state = self.random_state)  # Change: Random Seeds

        d_stats = self.statistics['subtype'][subtype]['duration']
        i_stats = self.statistics['subtype'][subtype]['intensity']

        dist = duration_decorator(getattr(np.random.RandomState(seed = self.random_state), d_stats['distribution'].lower()))  # Change: Random Seeds
        mean = to_timedelta(np.log(to_timedelta(d_stats['average']).total_seconds()) - 0.5)
        duration = dist(mean=mean).total_seconds()

        dist = getattr(np.random.RandomState(seed = self.random_state), i_stats['distribution'].lower())  # Change: Random Seeds
        low = i_stats['low']
        high = i_stats['high']
    ...
    def simulate(self, consumption, users=None, ind_enduse=None, pattern_num=1, day_begin = '', day_end = '', random_state: int = None):  # Change: Random Seeds, Fix Duration
        ...
        for j, user in enumerate(users):
            ...
            for i in range(freq):
                ...
                u = np.random.RandomState(seed = self.random_state).uniform()  # Change: Random Seeds
                start = np.argmin(np.abs(np.cumsum(prob_joint) - u)) + day_begin  # Change: Fix Duration
...
class Dishwasher(EndUse):
    ...
    def simulate(self, consumption, users=None, ind_enduse=None, pattern_num=1, day_begin = '', day_end = '', random_state: int = None):  # Change: Random Seeds, Fix Duration
        ...
        numusers = len(users) if len(users) <=5 else 5  # Change: Additional Users
        freq = self.fct_frequency(numusers=numusers)  # Change: Additional Users
        ...
        for i in range(freq):
            ...
            u = np.random.RandomState(seed = self.random_state).random() # Change: Random Seeds
            start = np.argmin(np.abs(np.cumsum(prob_joint) - u)) + day_begin  # Change: Fix Duration 
            end = start + duration
...
class KitchenTap(EndUse):
    ...
    def fct_duration_intensity(self):
        ...
        dist = getattr(np.random.RandomState(seed = self.random_state), d_stats['distribution'].lower())  # Change: Random Seeds
    ...
    def simulate(self, consumption, users=None, ind_enduse=None, pattern_num=1, day_begin = '', day_end = '', random_state: int = None):  # Change: Random Seeds, Fix Duration
        ...
        prob_usage = self.statistics['daily_pattern'].values
        ...
        numusers = len(users) if len(users) <=5 else 5
        freq = self.fct_frequency(numusers=numusers) 

        for i in range(freq):

            duration, intensity = self.fct_duration_intensity()
            u = np.random.RandomState(seed = self.random_state).uniform()  # Change: Random Seeds
            prob_joint = normalize(prob_user[day_begin:day_end] * prob_usage)  # Change: Fix Duration   # ToDo: Check if joint probability can be computed outside of for loop for all functions
            start = np.argmin(np.abs(np.cumsum(prob_joint) - u)) + day_begin  # Change: Fix Duration
            end = start + duration
            consumption[start:end, j, ind_enduse, pattern_num] = intensity
...
class Shower(EndUse):
    ...
    def fct_duration_intensity(self, age=None):
        ...
        distribution = getattr(np.random.RandomState(seed = self.random_state), d_stats['distribution'].lower())  # Change: Random Seeds
...
class WashingMachine(EndUse):
    ...
    def simulate(self, consumption, users=None, ind_enduse=None, pattern_num=1, day_begin = '', day_end = '', random_state: int = None):  # Change: Random Seeds, Fix Duration
        ...
        prob_joint = normalize(prob_user[day_begin:day_end] * prob_usage)
        ...
        for i in range(freq):
            ...
            u = np.random.RandomState(seed = random_state).random()           # Change: Random Seeds
            start = np.argmin(np.abs(np.cumsum(prob_joint) - u)) + day_begin  # Change: Fix Duration
            end = start + duration 
...
class Wc(EndUse):
    ...
    def fct_duration_intensity(self):
        ...
        if flush_interuption:
            v = np.random.RandomState(seed = self.random_state).random() * 100  # Change: Random Seeds
    ...
    def simulate(self, consumption, users=None, ind_enduse=None, pattern_num=1, day_begin = '', day_end = '', random_state: int = None):  # Change: Random Seeds
