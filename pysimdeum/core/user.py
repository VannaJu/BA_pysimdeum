...
from datetime import datetime # Change: Fix duration 
...
class Presence:
    ...
    def __post_init__(self) -> None:
        ...
        self.up = self.sample_single_property('_prob_getting_up', random_state = self.random_state) # Change: Random Seeds 

        self.sleep = self.up - self.sample_single_property('_prob_sleep', random_state = self.random_state) + pd.Timedelta(days=1) # Change: Random Seeds 

        self.go = self.sample_single_property('_prob_leaving_house', random_state = self.random_state) # Change: Random Seeds 

        if self.go < self.up:
            self.go = self.up + pd.Timedelta(minutes=30)

        self.home = self.go + self.sample_single_property('_prob_being_away', random_state = self.random_state) # Change: Random Seeds 
    ...
    def sample_single_property(self, prop: str, random_state: int = None) -> pd.Timedelta: # Change: Random Seeds 
        ...
        x = prob_fct.rvs(random_state = random_state) # Change: Random Seeds 
        x = int(np.round(x))
    ...
    def pdf(self, peak=0.65, normal=0.335, away=0.0, night=0.015):
        ...
        modu = len(index) - 1 # Change: Fix Duration, style no function changed
        
        pdf = pd.Series(index=index, dtype='float64')

        up = int((self.up.total_seconds()) / 60) % modu  # Change: Fix Duration, style no function changed
        up_p30 = int((up + 30)) % modu                   # Change: Fix Duration, style no function changed

        go = int(self.go.total_seconds() / 60) % modu    # Change: Fix Duration, style no function changed
        go_m30 = int(go - 30) % modu                     # Change: Fix Duration, style no function changed

        home = int(self.home.total_seconds() / 60) % modu  # Change: Fix Duration, style no function changed
        home_p30 = int(home + 30) % modu                   # Change: Fix Duration, style no function changed

        sleep = int(self.sleep.total_seconds() / 60) % modu # Change: Fix Duration, style no function changed
        sleep_m30 = int(sleep - 30) % modu                  # Change: Fix Duration, style no function changed
...
class User(Base):
    ...
    def compute_presence(self, duration = '1 day', weekday=True, statistics=None, peak=0.65, normal=0.335, away=0.0, night=0.15, random_state: [] = None): # Change: Fix Duration, Random Seeds
             
        
        # Change: Fix Duration
        timedelta = pd.to_timedelta(duration)
        days = pd.date_range(start=datetime.now().date(), end=datetime.now().date() + timedelta - pd.to_timedelta('1 d'))
        
        # Change: Random Seeds
        if random_state is None:
            random_state = [None] * len(days)
        
        # Change: Fix Duration 
        for i, day in enumerate(days):
        
            presence = Presence(user=self, weekday=day.dayofweek < 5, stats=statistics, random_state = random_state[i])
